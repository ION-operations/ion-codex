...
class KernelQuestionRouter:
    ...
    def __init__(self, *, lineage_manager: KernelNameLineageManager | None = None) -> None:
    ...
    def _build_question(
        ...
        *,
        ...
        if needed_from != "UNSPECIFIED":
            try:
                resolution = self._lineage_manager.resolve_required_name(
                    needed_from,
                    surface=NameIngressSurface.OPEN_QUESTION_ROUTE,
                )
            except KernelNameLineageError as exc:
                raise KernelQuestionRoutingError(
                    "Open-question routing requires an explicit live authority name: "
                    f"{needed_from}. {exc}"
                ) from exc
            needed_from = resolution.resolved_name or needed_from
            if resolution.raw_name != needed_from:
                needed_from_raw = resolution.raw_name
            needed_from_resolution_note = self._lineage_manager.render_resolution_note(resolution)
        ...
        return OpenQuestion(
            ...
            needed_from_raw=needed_from_raw,
            needed_from_resolution_note=needed_from_resolution_note,
