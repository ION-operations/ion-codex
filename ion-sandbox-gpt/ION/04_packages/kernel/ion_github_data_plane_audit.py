...
FAILURE_CLASSES = [
    "GITHUB_DATA_PLANE_ABSENT_BY_PACKAGE_POLICY",
...
def _load_product_manifest(shell_root: Path) -> dict[str, Any]:
    path = shell_root / "PRODUCT_MANIFEST.json"
    if not path.exists():
        return {"path": "PRODUCT_MANIFEST.json", "exists": False}
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        return {"path": "PRODUCT_MANIFEST.json", "exists": True, "valid_json": False, "error": str(exc)}
    return {
        ...
        "primary_loop": payload.get("primary_loop"),
        "hot_operational_root": payload.get("hot_operational_root"),
        "external_mcp_42_tool_lane": payload.get("external_mcp_42_tool_lane"),
        "sandbox_package_mode": payload.get("schema_id") == "ion.full_gpt_sandbox_agent_package.product_manifest.v1",
    }


def _github_absent_by_package_policy(product_manifest: dict[str, Any], git: dict[str, Any]) -> bool:
    return bool(product_manifest.get("sandbox_package_mode") and not git.get("git_present"))
...
def audit_github_data_plane(root: str | Path | None = None) -> dict[str, Any]:
    ...
    product_manifest = _load_product_manifest(shell_root)
    git = _git_probe(shell_root)
    absent_by_package_policy = _github_absent_by_package_policy(product_manifest, git)

    if absent_by_package_policy:
        findings.append("git_absent_by_gpt_sandbox_package_policy")
        warnings.append("github_data_plane_optional_for_gpt_sandbox_package")
    elif not git.get("git_present"):
    ...
    if absent_by_package_policy:
        readiness_blockers.append("github_data_plane_absent_by_package_policy")
    elif not git.get("git_present"):
    ...
    if findings:
        if any(finding == "git_absent_by_gpt_sandbox_package_policy" for finding in findings):
            failure_classification = "GITHUB_DATA_PLANE_ABSENT_BY_PACKAGE_POLICY"
        elif any(finding == "git_not_configured_at_shell_root" for finding in findings):
            failure_classification = "GITHUB_DATA_PLANE_NOT_CONFIGURED"
