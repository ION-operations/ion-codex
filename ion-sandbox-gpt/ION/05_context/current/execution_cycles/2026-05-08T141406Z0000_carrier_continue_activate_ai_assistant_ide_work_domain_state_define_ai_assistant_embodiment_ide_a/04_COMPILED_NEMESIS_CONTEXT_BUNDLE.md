# COMPILED NEMESIS CONTEXT BUNDLE

This V95 bundle is the same executable ION Carrier Role ContextPackage materialized under an explicit COMPILED_<ROLE>_CONTEXT_BUNDLE filename. Use it exactly as the generated context package; do not fall back to boot/MINI/CAPSULE/session-only onboarding.

- role: `NEMESIS`
- context_package_path: `ION/05_context/current/execution_cycles/2026-05-08T141406Z0000_carrier_continue_activate_ai_assistant_ide_work_domain_state_define_ai_assistant_embodiment_ide_a/04_nemesis_cursor_task_prompt.md`
- context_load_receipt_path: `ION/05_context/current/execution_cycles/2026-05-08T141406Z0000_carrier_continue_activate_ai_assistant_ide_work_domain_state_define_ai_assistant_embodiment_ide_a/04_nemesis_context_load_receipt.json`
...
# ION Carrier Role ContextPackage — NEMESIS

This prompt is the executable ContextPackage for a scheduled carrier role phase. It is not a generic instruction to 'read a file'. In single-carrier sandbox mode, the same LLM carrier mounts and executes this phase; in optional worker mode, an authorized worker may carry it. The carrier must complete the context-load transaction, prove it, then execute the bounded role pass.

## Mount

- agent_name: `NEMESIS`
- carrier: `scheduled_role_phase_carrier_slot`
- mounted_by: `local_STEWARD_carrier`
- authority_level: `AUDIT_AND_RELEASE_SENSITIVITY_REVIEW_BOUNDED`
- production_authority: `false`
- live_execution_authority: `false`
- workstream: `governance`
- session_packet_path: `ION/05_context/current/execution_cycles/2026-05-08T141406Z0000_carrier_continue_activate_ai_assistant_ide_work_domain_state_define_ai_assistant_embodiment_ide_a/04_nemesis_session.md`
- proposal_status: `PENDING_STEWARD_REVIEW`

## Mission

activate AI assistant / IDE work domain state: define AI assistant embodiment, IDE agent work space, specialist domains, protocols, templates, states, and agents; stage candidate current-context artifacts without acceptance or registry landing

## Agent Context System authority

This carrier role package is V82 Agent-Context-System aware. The role is not being booted from MINI/CAPSULE as primary authority; those files are continuity witnesses interpreted under the active package.
...
Before analysis or edits, use the host carrier's file-read mechanism on each file path below in this exact sequence. Directory and glob rows must be checked/listed when relevant. Parent-prefetched content later in this prompt is only a checksum-backed aid; it does not replace tool-read proof.
...
8. `ION/07_templates/reports/AUDIT.md` (file; required=false; status=missing_optional)
...
10. `ION/agents/nemesis/MINI.md` (file; required=false; status=missing_optional)
...
The parent carrier / Steward must reject the role/task return unless it starts with `### CONTEXT PROOF`, passes `kernel.ion_context_proof_gate` against this prompt's `*_context_load_receipt.json`, and passes `kernel.ion_template_action_gate`. A recap such as `I read the context file` is not onboarded evidence.
