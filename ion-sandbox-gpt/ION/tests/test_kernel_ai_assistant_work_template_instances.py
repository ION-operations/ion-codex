...
def test_instance_corpus_index_has_candidate_boundary_and_complete_coverage():
    ...
    assert index["template_spec_count"] == 7
    assert index["valid_instance_count"] == 7
    assert index["expected_rejection_count"] == 7
...
def test_validation_report_accepts_valid_instances_expected_rejections_and_boot_exercises():
    ...
    assert report["template_spec_count"] == 7
    assert report["valid_instance_count"] == 7
    assert report["expected_rejection_count"] == 7
    assert report["agent_boot_exercise_count"] == 7
    assert report["valid_instances_accepted"] == 7
    assert report["expected_rejections_accepted"] == 7
    assert report["agent_boot_exercises_accepted"] == 7
