...
class KernelIndex:
    ...
    def clear(self) -> None:
        ...
        self._executor_work_lifecycle_binding_receipts_by_scope: DefaultDict[tuple[str, str], set[RecordKey]] = defaultdict(set)
        ...
        self._mission_advisory_routing_receipts_by_scope: DefaultDict[tuple[str, str], set[RecordKey]] = defaultdict(set)
        self._mission_advisory_routing_receipts_by_profile: DefaultDict[MissionAdvisoryProfile, set[RecordKey]] = defaultdict(set)
        self._mission_advisory_manifest_route_binding_receipts_by_manifest: DefaultDict[str, set[RecordKey]] = defaultdict(set)
        self._mission_advisory_manifest_route_binding_receipts_by_source: DefaultDict[str, set[RecordKey]] = defaultdict(set)
        self._mission_advisory_horizon_binding_receipts_by_horizon: DefaultDict[str, set[RecordKey]] = defaultdict(set)
        self._mission_advisory_horizon_binding_receipts_by_source: DefaultDict[str, set[RecordKey]] = defaultdict(set)
        self._mission_advisory_planner_manifest_maintenance_receipts_by_source: DefaultDict[str, set[RecordKey]] = defaultdict(set)
        self._mission_advisory_planner_manifest_maintenance_disposition_receipts_by_source: DefaultDict[str, set[RecordKey]] = defaultdict(set)
        self._mission_advisory_planner_manifest_maintenance_disposition_receipts_by_action: DefaultDict[MissionAdvisoryPlannerManifestMaintenanceDispositionAction, set[RecordKey]] = defaultdict(set)
        self._mission_advisory_planner_manifest_maintenance_resolution_receipts_by_source: DefaultDict[str, set[RecordKey]] = defaultdict(set)
    ...
    def executor_work_lifecycle_binding_receipts_for_scope(
        self,
        scope_type: str,
        scope_ref: str,
    ) -> list[ExecutorWorkLifecycleBindingReceipt]:
        return self._typed_records(
            self._executor_work_lifecycle_binding_receipts_by_scope.get((scope_type, scope_ref), set()),
            ExecutorWorkLifecycleBindingReceipt,
        )

    def mission_advisory_routing_receipts_for_scope(
        self,
        scope_type: str,
        scope_ref: str,
    ) -> list[MissionAdvisoryRoutingReceipt]:
        return self._typed_records(
            self._mission_advisory_routing_receipts_by_scope.get((scope_type, scope_ref), set()),
            MissionAdvisoryRoutingReceipt,
        )

    def mission_advisory_routing_receipts_by_profile(
        self,
        profile: MissionAdvisoryProfile,
    ) -> list[MissionAdvisoryRoutingReceipt]:
        return self._typed_records(
            self._mission_advisory_routing_receipts_by_profile.get(profile, set()),
            MissionAdvisoryRoutingReceipt,
        )

    def mission_advisory_manifest_route_binding_receipts_for_manifest(
        self,
        manifest_id: str,
    ) -> list[MissionAdvisoryManifestRouteBindingReceipt]:
        return self._typed_records(
            self._mission_advisory_manifest_route_binding_receipts_by_manifest.get(manifest_id, set()),
            MissionAdvisoryManifestRouteBindingReceipt,
        )

    def mission_advisory_manifest_route_binding_receipts_for_source(
        self,
        mission_advisory_receipt_id: str,
    ) -> list[MissionAdvisoryManifestRouteBindingReceipt]:
        return self._typed_records(
            self._mission_advisory_manifest_route_binding_receipts_by_source.get(mission_advisory_receipt_id, set()),
            MissionAdvisoryManifestRouteBindingReceipt,
        )

    def mission_advisory_horizon_binding_receipts_for_horizon(
        self,
        horizon_id: str,
    ) -> list[MissionAdvisoryHorizonBindingReceipt]:
        return self._typed_records(
            self._mission_advisory_horizon_binding_receipts_by_horizon.get(horizon_id, set()),
            MissionAdvisoryHorizonBindingReceipt,
        )

    def mission_advisory_horizon_binding_receipts_for_source(
        self,
        mission_advisory_manifest_route_binding_receipt_id: str,
    ) -> list[MissionAdvisoryHorizonBindingReceipt]:
        return self._typed_records(
            self._mission_advisory_horizon_binding_receipts_by_source.get(mission_advisory_manifest_route_binding_receipt_id, set()),
            MissionAdvisoryHorizonBindingReceipt,
        )

    def mission_advisory_planner_manifest_maintenance_receipts_for_source(
        self,
        horizon_binding_receipt_id: str,
    ) -> list[MissionAdvisoryPlannerManifestMaintenanceReceipt]:
        return self._typed_records(
            self._mission_advisory_planner_manifest_maintenance_receipts_by_source.get(horizon_binding_receipt_id, set()),
            MissionAdvisoryPlannerManifestMaintenanceReceipt,
        )

    def mission_advisory_planner_manifest_maintenance_disposition_receipts_for_source(
        self,
        maintenance_receipt_id: str,
    ) -> list[MissionAdvisoryPlannerManifestMaintenanceDispositionReceipt]:
        return self._typed_records(
            self._mission_advisory_planner_manifest_maintenance_disposition_receipts_by_source.get(maintenance_receipt_id, set()),
            MissionAdvisoryPlannerManifestMaintenanceDispositionReceipt,
        )

    def mission_advisory_planner_manifest_maintenance_resolution_receipts_for_source(
        self,
        disposition_receipt_id: str,
    ) -> list[MissionAdvisoryPlannerManifestMaintenanceResolutionReceipt]:
        return self._typed_records(
            self._mission_advisory_planner_manifest_maintenance_resolution_receipts_by_source.get(disposition_receipt_id, set()),
            MissionAdvisoryPlannerManifestMaintenanceResolutionReceipt,
        )
    ...
    def _index_record(self, record: KernelRecord) -> None:
        ...
        if isinstance(record, WorkUnit):
            ...
            if record.scope_type == "WORK_UNIT":
                self._by_work_unit[record.scope_ref].add(key)
        elif isinstance(record, RootAuthorityBundleExerciseReceipt):
            pass
        elif isinstance(record, RootAuthorityBundleExternalReturnReceipt):
            pass
            ...
            if record.scope_type == "WORK_UNIT":
                self._by_work_unit[record.scope_ref].add(key)
        elif isinstance(record, MissionAdvisoryRoutingReceipt):
            if record.scope_type and record.scope_ref:
                scope_key = (record.scope_type, record.scope_ref)
                self._mission_advisory_routing_receipts_by_scope[scope_key].add(key)
                self._by_scope[record.scope_ref].add(key)
            if record.work_unit_id:
                self._by_work_unit[record.work_unit_id].add(key)
            self._mission_advisory_routing_receipts_by_profile[record.recommended_profile].add(key)
        elif isinstance(record, MissionAdvisoryManifestRouteBindingReceipt):
            self._mission_advisory_manifest_route_binding_receipts_by_manifest[record.manifest_id].add(key)
            self._mission_advisory_manifest_route_binding_receipts_by_source[record.source_mission_advisory_routing_receipt_id].add(key)
            self._by_scope[record.owner_scope_id].add(key)
        elif isinstance(record, MissionAdvisoryHorizonBindingReceipt):
            self._mission_advisory_horizon_binding_receipts_by_horizon[record.horizon_id].add(key)
            self._mission_advisory_horizon_binding_receipts_by_source[record.source_mission_advisory_manifest_route_binding_receipt_id].add(key)
            self._by_scope[record.owner_scope_id].add(key)
            if record.owner_scope_type == "WORK_UNIT":
                self._by_work_unit[record.owner_scope_id].add(key)
        elif isinstance(record, MissionAdvisoryPlannerManifestMaintenanceReceipt):
            self._mission_advisory_planner_manifest_maintenance_receipts_by_source[record.source_mission_advisory_horizon_binding_receipt_id].add(key)
            self._by_scope[record.owner_scope_id].add(key)
            if record.owner_scope_type == "WORK_UNIT":
                self._by_work_unit[record.owner_scope_id].add(key)
        elif isinstance(record, MissionAdvisoryPlannerManifestMaintenanceDispositionReceipt):
            self._mission_advisory_planner_manifest_maintenance_disposition_receipts_by_source[record.source_mission_advisory_planner_manifest_maintenance_receipt_id].add(key)
            self._mission_advisory_planner_manifest_maintenance_disposition_receipts_by_action[record.action].add(key)
            self._by_scope[record.owner_scope_id].add(key)
            if record.owner_scope_type == "WORK_UNIT":
                self._by_work_unit[record.owner_scope_id].add(key)
        elif isinstance(record, MissionAdvisoryPlannerManifestMaintenanceResolutionReceipt):
            self._mission_advisory_planner_manifest_maintenance_resolution_receipts_by_source[record.source_mission_advisory_planner_manifest_maintenance_disposition_receipt_id].add(key)
            self._by_scope[record.owner_scope_id].add(key)
            if record.owner_scope_type == "WORK_UNIT":
                self._by_work_unit[record.owner_scope_id].add(key)
            ...
            if record.scope_type == "WORK_UNIT":
                self._by_work_unit[record.scope_ref].add(key)
        elif isinstance(record, ExecutorWorkLifecycleBindingReceipt):
            scope_key = (record.scope_type, record.scope_ref)
            self._executor_work_lifecycle_binding_receipts_by_scope[scope_key].add(key)
            self._by_scope[record.scope_ref].add(key)
            if record.work_unit_id:
                self._by_work_unit[record.work_unit_id].add(key)
    ...
    def _deindex_record(self, record: KernelRecord) -> None:
        ...
        if isinstance(record, WorkUnit):
            ...
            scope_key = (record.scope_type, record.scope_ref)
            self._schedule_completion_release_receipts_by_scope.get(scope_key, set()).discard(key)
            self._by_scope.get(record.scope_ref, set()).discard(key)
            if record.scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.scope_ref, set()).discard(key)
            if record.work_unit_id:
            ...
            scope_key = (record.scope_type, record.scope_ref)
            self._schedule_settlement_receipts_by_scope.get(scope_key, set()).discard(key)
            self._by_scope.get(record.scope_ref, set()).discard(key)
            if record.scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.scope_ref, set()).discard(key)
            if record.work_unit_id:
            ...
            scope_key = (record.scope_type, record.scope_ref)
            self._schedule_lineage_archive_receipts_by_scope.get(scope_key, set()).discard(key)
            self._by_scope.get(record.scope_ref, set()).discard(key)
            if record.scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.scope_ref, set()).discard(key)
        elif isinstance(record, ScheduleLineageReplayReceipt):
            scope_key = (record.scope_type, record.scope_ref)
            self._schedule_lineage_replay_receipts_by_scope.get(scope_key, set()).discard(key)
            self._by_scope.get(record.scope_ref, set()).discard(key)
            if record.scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.scope_ref, set()).discard(key)
        elif isinstance(record, ScheduleResumeProjectionReceipt):
            scope_key = (record.scope_type, record.scope_ref)
            self._schedule_resume_projection_receipts_by_scope.get(scope_key, set()).discard(key)
            self._by_scope.get(record.scope_ref, set()).discard(key)
            if record.scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.scope_ref, set()).discard(key)
            if record.work_unit_id:
            ...
            if record.scope_type and record.scope_ref:
                self._mission_advisory_routing_receipts_by_scope.get((record.scope_type, record.scope_ref), set()).discard(key)
                self._by_scope.get(record.scope_ref, set()).discard(key)
            if record.work_unit_id:
                self._by_work_unit.get(record.work_unit_id, set()).discard(key)
            self._mission_advisory_routing_receipts_by_profile.get(record.recommended_profile, set()).discard(key)
        elif isinstance(record, MissionAdvisoryManifestRouteBindingReceipt):
            self._mission_advisory_manifest_route_binding_receipts_by_manifest.get(record.manifest_id, set()).discard(key)
            self._mission_advisory_manifest_route_binding_receipts_by_source.get(record.source_mission_advisory_routing_receipt_id, set()).discard(key)
            self._by_scope.get(record.owner_scope_id, set()).discard(key)
        elif isinstance(record, MissionAdvisoryHorizonBindingReceipt):
            self._mission_advisory_horizon_binding_receipts_by_horizon.get(record.horizon_id, set()).discard(key)
            self._mission_advisory_horizon_binding_receipts_by_source.get(record.source_mission_advisory_manifest_route_binding_receipt_id, set()).discard(key)
            self._by_scope.get(record.owner_scope_id, set()).discard(key)
            if record.owner_scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.owner_scope_id, set()).discard(key)
        elif isinstance(record, MissionAdvisoryPlannerManifestMaintenanceReceipt):
            self._mission_advisory_planner_manifest_maintenance_receipts_by_source.get(record.source_mission_advisory_horizon_binding_receipt_id, set()).discard(key)
            self._by_scope.get(record.owner_scope_id, set()).discard(key)
            if record.owner_scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.owner_scope_id, set()).discard(key)
        elif isinstance(record, MissionAdvisoryPlannerManifestMaintenanceDispositionReceipt):
            self._mission_advisory_planner_manifest_maintenance_disposition_receipts_by_source.get(record.source_mission_advisory_planner_manifest_maintenance_receipt_id, set()).discard(key)
            self._mission_advisory_planner_manifest_maintenance_disposition_receipts_by_action.get(record.action, set()).discard(key)
            self._by_scope.get(record.owner_scope_id, set()).discard(key)
            if record.owner_scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.owner_scope_id, set()).discard(key)
        elif isinstance(record, MissionAdvisoryPlannerManifestMaintenanceResolutionReceipt):
            self._mission_advisory_planner_manifest_maintenance_resolution_receipts_by_source.get(record.source_mission_advisory_planner_manifest_maintenance_disposition_receipt_id, set()).discard(key)
            self._by_scope.get(record.owner_scope_id, set()).discard(key)
            if record.owner_scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.owner_scope_id, set()).discard(key)
        elif isinstance(record, ContextPerfectContinuationReceipt):
            ...
            self._executor_work_lifecycle_binding_receipts_by_scope.get((record.scope_type, record.scope_ref), set()).discard(key)
            self._by_scope.get(record.scope_ref, set()).discard(key)
            if record.work_unit_id:
                self._by_work_unit.get(record.work_unit_id, set()).discard(key)
        elif isinstance(record, BranchClaimReceipt):
            ...
            parent_key = (record.parent_scope_type, record.parent_scope_ref)
            self._branch_horizon_sync_receipts_by_parent.get(parent_key, set()).discard(key)
            self._by_scope.get(record.parent_scope_ref, set()).discard(key)
            if record.parent_scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.parent_scope_ref, set()).discard(key)
            for work_unit_id in record.active_branch_work_unit_ids:
            ...
            parent_key = (record.parent_scope_type, record.parent_scope_ref)
            self._branch_reschedule_receipts_by_parent.get(parent_key, set()).discard(key)
            self._by_scope.get(record.parent_scope_ref, set()).discard(key)
            if record.parent_scope_type == "WORK_UNIT":
                self._by_work_unit.get(record.parent_scope_ref, set()).discard(key)
    ...
    def _record_key(self, record: KernelRecord) -> RecordKey:
        ...
        if isinstance(record, ExecutorWorkLifecycleBindingReceipt):
            return ("executor_work_lifecycle_binding_receipt", record.receipt_id)
        if isinstance(record, ExecutorCapability):
        ...
        if isinstance(record, RootAuthorityBundleExerciseReceipt):
            return ("root_authority_bundle_exercise_receipt", record.receipt_id)
        if isinstance(record, RootAuthorityBundleExternalReturnReceipt):
            return ("root_authority_bundle_external_return_receipt", record.receipt_id)
        if isinstance(record, ManualAutomationEquivalenceReceipt):
            return ("manual_automation_equivalence_receipt", record.receipt_id)
        if isinstance(record, MissionAdvisoryRoutingReceipt):
            return ("mission_advisory_routing_receipt", record.receipt_id)
        if isinstance(record, MissionAdvisoryManifestRouteBindingReceipt):
            return ("mission_advisory_manifest_route_binding_receipt", record.receipt_id)
        if isinstance(record, MissionAdvisoryHorizonBindingReceipt):
            return ("mission_advisory_horizon_binding_receipt", record.receipt_id)
        if isinstance(record, MissionAdvisoryPlannerManifestMaintenanceReceipt):
            return ("mission_advisory_planner_manifest_maintenance_receipt", record.receipt_id)
        if isinstance(record, MissionAdvisoryPlannerManifestMaintenanceDispositionReceipt):
            return ("mission_advisory_planner_manifest_maintenance_disposition_receipt", record.receipt_id)
        if isinstance(record, MissionAdvisoryPlannerManifestMaintenanceResolutionReceipt):
            return ("mission_advisory_planner_manifest_maintenance_resolution_receipt", record.receipt_id)
