"""Carrier continuation control loop for Cursor-hosted ION.

This is the safe ``continue/proceed/resume`` entrypoint. It does not perform
live external execution, does not call Cursor Task itself, and does not grant
production authority. It refreshes the file-backed carrier state that Cursor
must follow:
...
def build_carrier_turn_packet(
    *,
    ...
    if human_gates:
        ...
        intake_required_action = "Resolve open human gates before spawning role workers."
    elif spawned_rows:
        intake_status = "WAITING_FOR_TASK_RETURNS"
        intake_required_action = "Capture every Cursor Task output and run kernel.ion_carrier_task_return before Steward integration."
    else:
    ...
    return {
        ...
        "carrier_next_actions": [
            ...
            "For each row, paste the generated context_package_path into a Cursor Task worker; do not paste session_packet_path alone.",
            "Reject any Task return that does not begin with ### CONTEXT PROOF and include ### TEMPLATE ACTION PROOF.",
            "Validate each Task return against its context_load_receipt_path using kernel.ion_context_proof_gate.",
            "Validate each Task return's template/action section using kernel.ion_template_action_gate.",
            "Record each Task return with kernel.ion_carrier_task_return before Steward integration.",
        ...
        "forbidden_parent_chat_actions": [
            "Do not perform STEWARD/MASON/VIZIER/NEMESIS/etc. work in the parent carrier chat.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mode", default="manual-cursor", choices=("plan-only", "manual-cursor", "cursor-sdk"))
