    ...
    *,
    ...
    for key in ("production_authority", "live_execution_authority"):
        ...
        inflated = {
            ...
            if value not in {False, "UNDECLARED"}
        }
        if inflated:
            findings.append({
...
def build_gpt_sandbox_preflight(
    ...
    *,
    ...
    active_packet_carrier = str(active_packet.get("carrier", "")).lower().strip()
    requested_carrier = carrier.lower().strip()
    packet_caps = (
        _active_packet_capabilities(active_packet)
        if active_packet_carrier == requested_carrier
        else {}
    )
