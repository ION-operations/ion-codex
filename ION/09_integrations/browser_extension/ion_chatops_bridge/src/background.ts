...
async function actionGatewayConfig(): Promise<{ base: string; token: string }> {
...
async function actionGatewayJson(method: "GET" | "POST", path: string, payload?: unknown): Promise<any> {
...
async function validateWithDaemon(packet: IonActionPacket): Promise<ValidationResult> {
  return postJson("/actions/validate", packet) as Promise<ValidationResult>;
}

async function submitApproved(packet: IonActionPacket): Promise<any> {
...
function approvedPayload(payload: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...payload,
    approval: {
      approved: true,
...
async function requestBridgeApproval(sender: chrome.runtime.MessageSender, operation: string, summary: string, riskClass = "approval_required_mutation"): Promise<boolean> {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "ion_chatops_project_context_sync_zip") {
    (async () => {
      const projectPaths = Array.isArray(message.payload?.project_paths)
        ? message.payload.project_paths.map((path: unknown) => String(path ?? "").trim()).filter(Boolean)
        : [];
      const approved = await requestBridgeApproval(
        sender,
  ...
  (async () => {
    const packet = message.packet as IonActionPacket;
    const validation = await validateWithDaemon(packet);
    if (!validation.accepted) {
      sendResponse({ ok: false, stage: "validate", validation });
      return;
    }
    const approved = await chrome.tabs.sendMessage(sender.tab?.id ?? 0, {
